...
## 33-6: tagged × f64 昇格の共倒れ解消 (token-ring ベンチで特定した真因)

背景: 「tagged 有利ベンチ」(token-ring: 参照 move + identity + null チェック
のみのホットループ) がコンパイルされず JIT 0.8x。真因の連鎖:
(1) param 有界ループカウンタですら range 分析で i32 をわずかに超え、
