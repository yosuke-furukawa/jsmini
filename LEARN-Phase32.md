# LEARN-Phase32.md — クラスタコンパイル: 呼び出し境界を消して NS 17x
...
## 余談: Octane 自体は 2017 年に retire 済み

V8 チームは 2017-04 に Octane を廃止している
(https://v8.dev/blog/retiring-octane)。理由は「ベンチマークの寿命」:
- Octane 向け最適化が実世界を**悪化**させ始めた (積極的インライニングの
  コンパイルコスト、Octane 特化ヒューリスティックが Ember で性能低下、
  Box2DWeb の**バグを悪用**した +15% 等)
- ピーク性能偏重で、実世界の支配項 (パース/ロード/コールドコード/
  モダンフレームワーク) を測っていない

それでも jsmini に Octane が有効なのは、私たちが**曲線の反対側**に
いるから。Phase 30 の 5 バグは全部汎用の言語バグ、Phase 32 の機構も
汎用 — 「勝ち方が一般化している間はベンチに価値がある。ベンチ専用の
細工でしか勝てなくなったら retire の時」という V8 の判断基準が、
そのまま健全性チェックになる。Phase 31 で見た deltablue の
「初回コンパイル時間 vs ピーク性能」の緊張は、V8 が指摘した罠の縮小版
(うちの bench は wall-time 直測でコンパイル時間込み = その点は本家より
正直)。V8 の「実世界へ」に相当する次の一手は test262 (PLAN-v7 P3)。
