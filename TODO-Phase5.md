...
- [x] 型特殊化: フィードバックの推奨型 (int32/uint32 → i32, f64 → f64) でコンパイル
- [x] i32 パス: `i32.add`, `i32.mul`, `i32.lt_s`, `i32.const` (LEB128)
- [x] f64 パス: `f64.add`, `f64.mul`, `f64.lt`, `f64.const`
- [x] JitManager が全引数同一型なら i32 特殊化、混在なら f64
- [x] テスト (3件): 整数 i32、小数 f64、乗算 i32
...
- [x] 型ガード: Wasm 呼び出し前に TypeScript 側で全引数が number か確認
- [x] 型不一致 → Wasm キャッシュ無効化 + `deoptimized` セットに追加 (再 JIT 不可)
- [x] 脱最適化後は永続的に VM で実行
- [x] deoptLog に `[DEOPT] func: expected number but got (types)` を記録
- [x] `vmEvaluate({ collectDeopt: true })` で `VMResult.deoptLog` を取得
- [x] テスト (3件): 文字列渡しでフォールバック、脱最適化後の継続実行、ログ記録
...
- [x] `--trace-tier` フラグで実行層の切り替えをログ出力
- [ ] Playground に実行層の表示を追加 — 後回し
...
- [x] `npm run bench` を3層対応に拡張 (JIT eligible ベンチマーク追加)
- [ ] 結果を README に記載
- [x] 3層比較完了: TW 6.1ms → VM 2.9ms (2.1x) → JIT 4.7ms (コンパイルコスト込み)
